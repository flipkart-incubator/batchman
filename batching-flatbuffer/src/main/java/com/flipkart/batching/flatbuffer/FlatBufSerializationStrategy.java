...
public class FlatBufSerializationStrategy implements SerializationStrategy<Data, Batch> {
    ...
    public Batch deserializeBatch(byte[] bytes) throws IOException {
        ...
        if (rootAsBatch.maxBatchSize() != 0) {
            finalBatch = new SizeBatch(dataCollection, (int) rootAsBatch.maxBatchSize());
        }

        if (rootAsBatch.maxBatchSize() != 0 && rootAsBatch.maxTimeout() != 0) {
